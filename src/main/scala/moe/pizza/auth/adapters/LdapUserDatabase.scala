...
/**
  ...
  */
class LdapUserDatabase(client: LdapClient,
  ...
  override def authenticateUser(uid: String, password: String): Option[Pilot] = {
    ...
    }).filter(_ == true).flatMap(_ => getUser(uid))
  }

  override def getUser(uid: String): Option[Pilot] = {
    client.withConnection { connection =>
      connection
        .filter(basedn, s"(&(uid=$uid)(objectclass=pilot))")
        .toList
        .headOption
        .map(_.toMap)
        .map(Pilot.fromMap)
  ...
  override def getUsers(filter: String): List[Pilot] = {
    client.withConnection { c =>
      c.filter(basedn, s"(&(objectclass=pilot)($filter))")
        .toList
        .map(_.toMap)
        .map(Pilot.fromMap)
    }
  ...
  override def getAllUsers(): Seq[Pilot] = {
    client.withConnection { connection =>
      connection
        .filter(basedn, "(objectclass=pilot)")
        .toSeq
        .map(_.toMap)
        .map(Pilot.fromMap)
    }
