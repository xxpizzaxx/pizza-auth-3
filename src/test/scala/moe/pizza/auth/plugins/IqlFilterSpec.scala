...
class IqlFilterSpec extends FlatSpec with MustMatchers {
  ...
  "IqlFilter" should "filter users using IQL" in {
    ...
    iqlf.filter(input, "(.corporation == \"mycorp\") && (.metadata.meta == \"metafiel\") && (.alliance == \"myalliance\")") must equal(List.empty[Pilot])
    iqlf.filter(input, "(.corporation == \"mycorp\") && (\"group1\" in .authGroups)") must equal(List(p, p2))
    iqlf.filter(input, "(.corporation == \"mycorp\") && (\"group2\" in .authGroups)") must equal(List(p2))
  }
